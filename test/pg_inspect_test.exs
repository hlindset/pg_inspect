defmodule PgInspectTest do
  ...
  describe "analyze/1 for SELECT statements" do
    test "parses simple SELECT query" do
      {:ok, result} = PgInspect.analyze("SELECT 1")
    ...
    test "handles parse errors" do
      {:error, %{message: message}} =
        PgInspect.analyze("CREATE RANDOM ix_test ON contacts.person;")

      assert message =~ "syntax error at or near \"RANDOM\""

      {:error, %{message: message}} = PgInspect.analyze("SELECT 'ERR")
    ...
    test "parses empty queries" do
      {:ok, result} = PgInspect.analyze("-- nothing")
    ...
    test "parses nested SELECT in FROM clause" do
      {:ok, result} = PgInspect.analyze("SELECT u.* FROM (SELECT * FROM users) u")
  ...
  describe "analyze/1 for set operations" do
    ...
    test "parses really deep queries" do
      ...
      {:ok, result} = PgInspect.analyze(query_text)
  ...
  describe "parse" do
    test "parses ALTER TABLE" do
      {:ok, result} = PgInspect.analyze("ALTER TABLE test ADD PRIMARY KEY (gid)")
      assert_tables_eq(result, ["test"])
      assert_ddl_tables_eq(result, ["test"])
    end

    test "parses ALTER VIEW" do
      {:ok, result} = PgInspect.analyze("ALTER VIEW test SET (security_barrier = TRUE)")
      assert_tables_eq(result, ["test"])
      assert_ddl_tables_eq(result, ["test"])
    end

    test "parses ALTER INDEX" do
      {:ok, result} = PgInspect.analyze("ALTER INDEX my_index_name SET (fastupdate = on)")
      assert_tables_eq(result, ["my_index_name"])
      assert_ddl_tables_eq(result, ["my_index_name"])
    end

    test "parses SET" do
      {:ok, result} = PgInspect.analyze("SET statement_timeout=0")
      assert_tables_eq(result, [])
    end

    test "parses SHOW" do
      {:ok, result} = PgInspect.analyze("SHOW work_mem")
      assert_tables_eq(result, [])
    end

    test "parses COPY" do
      {:ok, result} = PgInspect.analyze("COPY test (id) TO stdout")
      assert_tables_eq(result, ["test"])
      assert_select_tables_eq(result, ["test"])
    end

    test "parses DROP TABLE" do
      {:ok, result} = PgInspect.analyze("drop table abc.test123 cascade")
      assert_tables_eq(result, ["abc.test123"])
      assert_ddl_tables_eq(result, ["abc.test123"])
    end

    test "parses VACUUM" do
      {:ok, result} = PgInspect.analyze("VACUUM my_table")
      assert_tables_eq(result, ["my_table"])
      assert_ddl_tables_eq(result, ["my_table"])
    end

    test "parses EXPLAIN" do
      {:ok, result} = PgInspect.analyze("EXPLAIN DELETE FROM test")
      assert_tables_eq(result, ["test"])
    end

    test "parses SELECT INTO" do
      {:ok, result} = PgInspect.analyze("CREATE TEMP TABLE test AS SELECT 1")
      assert_tables_eq(result, ["test"])
      assert_ddl_tables_eq(result, ["test"])
    end
    ...
    test "parses LOCK" do
      {:ok, result} =
        PgInspect.analyze("LOCK TABLE public.schema_migrations IN ACCESS SHARE MODE")
    ...
    test "parses CREATE TABLE" do
      {:ok, result} = PgInspect.analyze("CREATE TABLE test (a int4)")
      assert_tables_eq(result, ["test"])
      assert_ddl_tables_eq(result, ["test"])
    end
    ...
    test "parses CREATE SCHEMA" do
      {:ok, result} = PgInspect.analyze("CREATE SCHEMA IF NOT EXISTS test AUTHORIZATION joe")
      assert_tables_eq(result, [])
    end

    test "parses CREATE VIEW" do
      {:ok, result} = PgInspect.analyze("CREATE VIEW myview AS SELECT * FROM mytab")
    ...
    test "parses REFRESH MATERIALIZED VIEW" do
      {:ok, result} = PgInspect.analyze("REFRESH MATERIALIZED VIEW myview")
    ...
    test "parses CREATE RULE" do
      {:ok, result} =
        PgInspect.analyze("CREATE RULE shoe_ins_protect AS ON INSERT TO shoe DO INSTEAD NOTHING")
    ...
    test "parses DROP SCHEMA" do
      {:ok, result} = PgInspect.analyze("DROP SCHEMA myschema")
      assert_tables_eq(result, [])
    end

    test "parses DROP VIEW" do
      {:ok, result} = PgInspect.analyze("DROP VIEW myview, myview2")
    ...
    test "parses DROP INDEX" do
      {:ok, result} = PgInspect.analyze("DROP INDEX CONCURRENTLY myindex")
      assert_tables_eq(result, [])
    end

    test "parses DROP RULE" do
      {:ok, result} = PgInspect.analyze("DROP RULE myrule ON mytable CASCADE")
      assert_tables_eq(result, ["mytable"])
      assert_ddl_tables_eq(result, ["mytable"])
    end

    test "parses DROP TRIGGER" do
      {:ok, result} = PgInspect.analyze("DROP TRIGGER IF EXISTS mytrigger ON mytable RESTRICT")
      assert_tables_eq(result, ["mytable"])
      assert_ddl_tables_eq(result, ["mytable"])
    end

    test "parses GRANT" do
      {:ok, result} = PgInspect.analyze("GRANT INSERT, UPDATE ON mytable TO myuser")
      assert_tables_eq(result, ["mytable"])
      assert_ddl_tables_eq(result, ["mytable"])
    end

    test "parses REVOKE" do
      {:ok, result} = PgInspect.analyze("REVOKE admins FROM joe")
      assert_tables_eq(result, [])
    end

    test "parses TRUNCATE" do
      {:ok, result} = PgInspect.analyze(~s|TRUNCATE bigtable, "fattable" RESTART IDENTITY|)
    ...
    test "correctly finds called functions" do
      {:ok, result} = PgInspect.analyze("SELECT foo.testfunc(1);")
      assert_tables_eq(result, [])
      assert_functions_eq(result, ["foo.testfunc"])
    ...
    test "correctly finds dropped functions" do
      {:ok, result} = PgInspect.analyze("DROP FUNCTION IF EXISTS foo.testfunc(x integer);")
      assert_tables_eq(result, [])
      assert_functions_eq(result, ["foo.testfunc"])
      assert_ddl_functions_eq(result, ["foo.testfunc"])
      assert_call_functions_eq(result, [])
    ...
    test "correctly finds renamed functions" do
      {:ok, result} =
        PgInspect.analyze("ALTER FUNCTION foo.testfunc(integer) RENAME TO testfunc2;")
    ...
    test "correctly separates CTE names from table names" do
      {:ok, result} =
        PgInspect.analyze("WITH cte_name AS (SELECT 1) SELECT * FROM table_name, cte_name")
  ...
  test "handles DROP TYPE" do
    {:ok, result} = PgInspect.analyze("DROP TYPE IF EXISTS repack.pk_something")
    assert_tables_eq(result, [])
  end

  test "handles COPY" do
    {:ok, result} = PgInspect.analyze("COPY (SELECT test FROM abc) TO STDOUT WITH (FORMAT 'csv')")
  ...
  test "parses CREATE TEMP TABLE" do
    {:ok, result} =
      PgInspect.analyze("CREATE TEMP TABLE foo AS SELECT 1;")
  ...
  describe "filter_columns" do
    test "finds unqualified names" do
      {:ok, result} = PgInspect.analyze("SELECT * FROM x WHERE y = $1 AND z = 1")
      assert_filter_columns_eq(result, [{nil, "y"}, {nil, "z"}])
    end

    test "finds qualified names" do
      {:ok, result} = PgInspect.analyze("SELECT * FROM x WHERE x.y = $1 AND x.z = 1")
    ...
    test "recognizes boolean tests" do
      {:ok, result} = PgInspect.analyze("SELECT * FROM x WHERE x.y IS TRUE AND x.z IS NOT FALSE")
      assert_filter_columns_eq(result, [{"x", "y"}, {"x", "z"}])
    end

    test "finds COALESCE argument names" do
      {:ok, result} = PgInspect.analyze("SELECT * FROM x WHERE x.y = COALESCE(z.a, z.b)")
  ...
  describe "truncate" do
    test "convenience wrapper for truncate works" do
      ...
      {:ok, result} = PgInspect.analyze(query)
      {:ok, truncated} = PgInspect.truncate(result, 40)
  ...
  defp assert_statement_types_eq(result, expected) do
    assert PgInspect.statement_types(result) == expected
  end

  defp assert_tables_eq(result, expected) do
    assert Enum.sort(PgInspect.tables(result)) == Enum.sort(expected)
  end

  defp assert_select_tables_eq(result, expected) do
    assert Enum.sort(PgInspect.select_tables(result)) == Enum.sort(expected)
  end

  defp assert_ddl_tables_eq(result, expected) do
    assert Enum.sort(PgInspect.ddl_tables(result)) == Enum.sort(expected)
  end

  defp assert_dml_tables_eq(result, expected) do
    assert Enum.sort(PgInspect.dml_tables(result)) == Enum.sort(expected)
  end
  ...
  defp assert_functions_eq(result, expected) do
    assert Enum.sort(PgInspect.functions(result)) == Enum.sort(expected)
  end

  defp assert_call_functions_eq(result, expected) do
    assert Enum.sort(PgInspect.call_functions(result)) == Enum.sort(expected)
  end

  defp assert_ddl_functions_eq(result, expected) do
    assert Enum.sort(PgInspect.ddl_functions(result)) == Enum.sort(expected)
  end

  defp assert_filter_columns_eq(result, expected) do
    assert Enum.sort(PgInspect.filter_columns(result)) == Enum.sort(expected)
  end
